...
def entry(state: State):
    """
    チェックポイントに前回のrouteが残っていれば、そのまま該当エージェントへ。
    routeが無い or /switch コマンドがあれば classify へ。
    """
    # 直近のユーザ発話を拾う
    last_user = None
    ...
    # 明示リセットコマンド
    if last_user and isinstance(last_user, str) and last_user.strip().lower().startswith("/switch"):
        return {"route": None}  # routeをクリア → classifyに行く

    return {}  # 状態は変えずに条件分岐で飛ばす


def entry_condition(state: State):
    """
    entryの直後に呼ばれる分岐関数。
    - routeがNone/未設定 → classify
    - routeが設定済み → そのまま該当agentへ（weather/web/chat）
    """
    r = state.get("route")
    if not r:
        return "classify"
    # 安全側：未知文字列はclassifyへ
    if r not in {"weather", "web", "chat"}:
        return "classify"
    return f"{r}_agent"
...
builder.add_conditional_edges(
    "entry", entry_condition, ["classify", "weather_agent", "web_agent", "chat_agent"]
)
